...
--Numero d auteur inconnu
SELECT COUNT(*) INTO C2
FROM CHANSON
WHERE idAuteur = vIdAuteur;
IF (C2<1) THEN
	RAISE AuteurInexistant;
END IF;
--Numero de Compositeur inconnu
SELECT COUNT(*) INTO C3
FROM CHANSON
WHERE idCompositeur = vIdCompo;
IF (C3<1) THEN
	RAISE CompositeurInexistant;
END IF;
...
When AuteurInexistant then
	DBMS_OUTPUT.PUT_LINE('Numero d auteur inconnu dans la base, insertion non effectuee');
	DBMS_OUTPUT.PUT_LINE ('Code erreur ' || SQLCODE) ;
	DBMS_OUTPUT.PUT_LINE ('Message d''erreur ' || SQLERRM) ; 

When CompositeurInexistant then
	DBMS_OUTPUT.PUT_LINE('Numero de compositeur inconnu dans la base, insertion non effectuee');
	DBMS_OUTPUT.PUT_LINE ('Code erreur ' || SQLCODE) ;
	DBMS_OUTPUT.PUT_LINE ('Message d''erreur ' || SQLERRM) ; 
